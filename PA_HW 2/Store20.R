...
require(astsa)
train <- read.csv("Store20_Prob2.csv")
colnames(train)
#Removing Date
train<-train[-3]
...
summary(fit <- lm(Total_Sales ~Temperature + MarkDown2+ MarkDown3+ month, data = train_data))
...
summary(fit1 <- lm(Total_Sales ~Temperature + MarkDown3+ month, data = train_data))
series2 = ts(resid(fit1), frequency = 52, start= c(2010,5), end=c(2012,13))
plot(series2)
acf2(resid(fit1),max.lag = 100) # AR0 MA0 
